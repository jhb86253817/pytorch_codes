...
void THNN_(unfolded_copy)(
          ...
          int padW,
          int padH,
          int nInputPlane, // number of input channels
          int inputWidth,
          int inputHeight,
          int outputWidth,
          int outputHeight)
{
  ...
  at::parallel_for(0, (int64_t)nInputPlane*kH*kW, 0, [&](int64_t start, int64_t end) {
    for (auto k = start; k < end; k++) {
      int64_t nip = k / (kH*kW); // nth input channel
      int64_t rest = k % (kH*kW); // extra kernels on nth input channel
      int64_t kh = rest / kW; // nth kernel row within rest
      int64_t kw = rest % kW;
      int x, y;
      int64_t ix, iy;
      // the nth row of target
      ...
      // the nth plane of input image
      scalar_t *src = input_data + nip*((size_t)inputHeight*inputWidth);
      if (padW > 0 || padH > 0) {
        int64_t lpad,rpad;
        for(y = 0; y < outputHeight; y++) { // y index of output
          iy = (int64_t)y*dH - padH + kh; // y index of input
          if (iy < 0 || iy >= inputHeight) {
            memset(dst+(size_t)y*outputWidth, 0, sizeof(scalar_t)*outputWidth);
          } else {
            if (dW==1){
               ix = 0 - padW + kw; // x index of input
               lpad = fmaxf(0,padW-kw); // how many to pad on left
               rpad = fmaxf(0,padW-(kW-kw-1)); // how many to pad on right
              ...
              // to adapt to mask conv, change the whole row to mask indices
              for (x=0; x<outputWidth; x++){ // x index of output
                 ix = (int64_t)x*dW - padW + kw;
                 if (ix < 0 || ix >= inputWidth)
